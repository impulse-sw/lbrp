...
pub(crate) async fn sign_up_step1(id: String) -> CResult<(String, Vec<u8>)> {
  ...
  let state = resp
    .headers()
    .get(c3a_client_sdk::PREREGISTER_HEADER)
...
) -> CResult<c3a_client_sdk::TokenTriple> {
  let triple = reqwest::Client::new()
    .post(endpoint("/--inner-lbrp-auth/sign-up-step2"))
    .header(c3a_client_sdk::PREREGISTER_HEADER, state)
    .json(&RegisterRequest {
      id,
      password,
      cdpub: Some(cdpub),
    ...
    .await?
    .error_for_status()?
    .json::<c3a_client_sdk::TokenTriple>()
    .await?;
...
pub(crate) async fn check_auth() -> bool {
  reqwest::Client::new()
    .get("/")
    .authorize(endpoint("/--inner-lbrp-auth/checkup"))
    .await
    .is_ok()
}
